...
import { sleep, getTickers, getHastags} from '@lib/utils';
...
import { FeelingModal } from '@components/modal/feeling-modal';
import { FeelingCombobox } from '@components/input/feeling-combobox';
...
export function Input({
  ...
  const {
    ...
    openModal: openFeelingModal,
    closeModal: closeFeelingModal
  } = useModal();
  ...
  const sendTweet = async (): Promise<void> => {
    ...
    if(tickers.length) {
      ...
      // check tweet text to parse different $tags
    const hashtags = getHashtags(inputValue.trim() || '');
    if(hashtags.length) {
      await manageHashtags('add', hashtags, tweetId);
    }
 ...
 const toggleShowLocation = () => {
  ...
  const onSetFeeling = (e: FormEvent<HTMLFormElement>): void => {
    e.preventDefault();
    setFeeling(feelingInputValue);
    closeFeelingModal();
  ...
  const cancelSetLocation = (): void => {
    ...
    const cancelSetFeeling = (): void => {
    setfeelingInputValue('');
       if (showFeeling && feeling === '') {
  ...
  const handleFeelingChange = (feeling: string): void =>
    setfeelingInputValue(feeling);
    ...
    {loading && (
        ...
        >
          ...
          <div className='flex w-full flex-col gap-4'>
           ...
           onToggleShowFeeling={toggleShowFeeling}
            >
              {isUploadingImages && (
                <ImagePreview
      ...
      >
        ...
        modalClassName='flex flex-col gap-6 max-w-xl bg-main-background w-full p-8 rounded-2xl flex-grow'
        open={feelingModalOpen}
        closeModal={cancelSetFeeling}
      >
        <FeelingModal
          loading={loading}
          onSetFeeling={onSetFeeling}
          removeSetFeeling={removeSetFeeling}
